 ...
 * @version   SVN:$id
 */
class Xml implements JdbcAdapterInterface
{
	public function getConexion($host,$port,$database,$user,$password)
	{
		...
		if ($exist)
		{			
			$JRXmlDataSource = new java(
				'net.sf.jasperreports.engine.data.JRXmlDataSource',realpath($host));
			
			return $JRXmlDataSource;
		}else{
   			echo "El archivo $host no existe";
			return false;
